  ...
  case $ensure {
    'present' : {
      ...
      exec { "create_piwik_site_${name}":
        ...
        user    => 'root',
        unless  => "/usr/bin/curl --silent --insecure --header 'Host: ${piwik::url}' 'http://localhost/?module=API&method=SitesManager.getAllSites&token_auth=${token_auth_final}&format=json' | /bin/grep ${name}"
      }
    }
    'absent': {
      ...
      exec { "create_piwik_site_${name}":
        command => "/usr/bin/curl --silent --insecure --header 'Host: ${piwik::url}' 'http://localhost/?module=API&method=SitesManager.deleteSite&idSite=${id}&token_auth=${token_auth_final}'",
        user    => 'root',
      }
    }
